...
As presented in @fig-2-1-3, `score` is highly correlated with `game_duration`, `coins_collected`, `ufos_shot`, and `pipes_passed.` This confirms that players who survive longer tend to accumulate more points and achievements, validating the game's internal scoring logic.
...
@fig-2-2-2 reveals that deaths caused by "enemy_bullet" and "ufo_collision" are associated with significantly higher median scores. This implies that players who are skilled enough to survive the initial pipes and engage with enemies tend to be higher-performing players overall. Conversely, dying by hitting the "ground" is associated with the lowest scores, often zero.
...
@tbl-2-2-1 provides a statistical summary of the "cost" associated with each type of death. It calculates the mean and median score at the moment of failure. The key insight is that deaths from ufo_collision (mean score: 30.3) and enemy_bullet (mean score: 28.3) occur to players who have already accumulated a high score. In contrast, dying by hitting the ground (mean score: 0.8) almost exclusively happens to players with little to no score. 
...
@fig-2-3-1 reveals fascinating player archetypes. High aggressiveness correlates moderately with efficiency, suggesting spam-shooting can be effective. However, accuracy shows weak correlations with other metrics, indicating that aiming skill operates independently from other play style elements. The complexity metric (sum of key actions) shows strong positive relationships with most performance indicators, confirming that active players generally perform better.
...
@fig-2-4-1 reveals a strong, positive, and non-linear relationship between how long a player survives (`game_duration`) and their final score. The trendline shows that the score appears to accelerate as game duration increases. This suggests that players who survive longer become more efficient at scoring. The widening cone shape of the data points indicates that while survival is a prerequisite for a high score, among players who last a long time, there is a greater variance in scoring ability. 

@fig-2-4-2 show that players in higher skill tiers (e.g., pink) consistently achieve a higher number of UFO hits for a given number of bullets fired. Their data points are positioned higher on the chart. Conversely, lower-skilled players (e.g., green, orange) are clustered at the bottom, indicating that they may be firing indiscriminately with low accuracy.
